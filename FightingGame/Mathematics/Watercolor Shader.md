...
// Native glslViewer uniform bindings
uniform float u_time;
uniform vec2 u_resolution;

// Anti-aliasing control: 1 = off (performance), 2 = on (quality)
#define AA 0
...
{
  q += 0.05 * sin(vec2(0.11, 0.13) * (u_time * 0.1) + length(q) * 4.0);
...
void main()
{
  ...
  for (int mi = 0; mi < AA; mi++)
    for (int ni = 0; ni < AA; ni++)
    {
      ...
      vec2 q = (2.0 * (gl_FragCoord.xy + of) - u_resolution) / u_resolution.y;
      #else
